...
python3 experiments/openxc7-macos/build_ddr_bios.py \
  --with-sd --fast-sd --sd-writable --native-bist \
  --registered-bank --register-command-buffers \
  --config projects/microsd-emulator/ddr/arty-bios-80-depth2.yml \
  --reset-source button0 --route-seeds 4 8 10 \
...
direct SD outputs and a configuration-frame round trip. A failed run may leave
old artifacts: require a successful exit and verify the matching `result.json`
and bitstream hash before programming. Seed 8 passed the prior qualified build;
placement results can change with sources, compiler output and dependencies.

The [2026-09-10 record](../../../docs/hardware/arty-nextpnr-0.9.4-timing-refactor-2026-09-10.json)
...
The GKD connection is `root@192.168.50.39`; the verified host-key file on the
Mac is `/private/tmp/gkd-microsd-known-hosts`. Only **`mmc1` / `2a310000.mmc`**
is the external test controller. Internal eMMC is `mmc0`; Wi-Fi is `mmc2`.
...
assert (host / 'mmc1:0001/cid').read_text().strip() == \
    '7f52425350414445101234567801916b'
assert '/dev/mmcblk1' not in Path('/proc/mounts').read_text()
...
uv run python tools/microsd_image.py --status --port /dev/cu.usbserial-210319B0C1CC1
```
...
uv run python tools/microsd_image.py small-image.bin \
  --port /dev/cu.usbserial-210319B0C1CC1
```
...
assert not (base / '2a310000.mmc').exists()
...
assert int(caps.read_text().strip(), 16) == 0x400c0007
...
Copy `tools/microsd_verify_linux.py`, `microsd_verify_linux_rw.py`,
`microsd_verify_linux_fs.py` and `microsd_stress_linux.py` into one temporary
directory on the GKD (create it with `mktemp -d`). Run the following there.
...
checker with the new width and `--read-only`.
