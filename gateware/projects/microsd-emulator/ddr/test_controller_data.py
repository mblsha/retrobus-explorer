...
class ControllerDataTests(unittest.TestCase):
    def exercise(
        ...
        settings = controller.ControllerSettings(
            ...
            else (4 if experimental or short_buffered else 16),
            cmd_buffer_buffered=experimental or short_buffered,
            with_auto_precharge=not experimental,
            **({"refresh_cls": make_refresher()} if experimental else {}),
        )
        cls = (
            make_bankmachine()
            if experimental or registered_bank
            else controller.BankMachine
        )
    ...
    def test_continuous_depth2_buffered_registered_bank_data(self):
        self.exercise(
            False, short_buffered=True, registered_bank=True, continuous=True, depth=2
        )

    def test_continuous_depth1_registered_bank_data(self):
        self.exercise(False, registered_bank=True, continuous=True, depth=1)

    def test_continuous_unbuffered_upstream_data(self):
        self.exercise(False, continuous=True)

    def test_continuous_unbuffered_registered_bank_data(self):
        self.exercise(False, registered_bank=True, continuous=True)
    ...
    def test_short_buffered_upstream_data(self):
        self.exercise(False, short_buffered=True)

    def test_short_buffered_registered_bank_data(self):
        self.exercise(False, short_buffered=True, registered_bank=True)
