...
def ft_capture_enabled(argv: list[str]) -> bool:
    return not has_flag(argv, "--no-ft-capture")
...
def main(argv: list[str]) -> int:
    if len(argv) < 2:
        raise SystemExit(
            "usage: mvp_imem_ce1_seeded_abs_read_chain.py plan|parse [args...]"
        )
    command = argv[1]
    if command == "plan":
        return emit_json(
    ...
    if command == "parse":
        if len(argv) < 3:
            raise SystemExit(
                "usage: mvp_imem_ce1_seeded_abs_read_chain.py parse RESULT.json [count]"
            )
