...
def main(argv: list[str]) -> int:
    if len(argv) < 2:
        raise SystemExit(
            "usage: pushu_a_roundtrip_then_popu_imr_a0_pre_sc_chain.py plan|parse [args...]"
        )
    command = argv[1]
    if command == "plan":
    ...
    if command == "parse":
        if len(argv) < 3:
            raise SystemExit(
                "usage: pushu_a_roundtrip_then_popu_imr_a0_pre_sc_chain.py parse RESULT.json [count]"
            )
