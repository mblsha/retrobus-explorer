...
ASM_SOURCE = PROJECT_ROOT / "asm" / "card_rom_iocs_beep_1_123_100_fffdc.asm"
...
def build_plan() -> dict[str, object]:
    return {
        "name": "iocs_beep_1_123_100_fffdc",
...
def main(argv: list[str]) -> int:
    if len(argv) < 2:
        raise SystemExit("usage: iocs_beep_1_123_100_fffdc.py plan|parse [args...]")
    command = argv[1]
    if command == "plan":
        return emit_json(build_plan())
    if command == "parse":
        if len(argv) < 3:
            raise SystemExit("usage: iocs_beep_1_123_100_fffdc.py parse RESULT.json")
