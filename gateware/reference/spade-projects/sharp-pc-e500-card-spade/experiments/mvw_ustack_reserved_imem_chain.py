...
def main(argv: list[str]) -> int:
    if len(argv) < 2:
        raise SystemExit("usage: mvw_ustack_reserved_imem_chain.py plan|parse [args...]")
    command = argv[1]
    if command == "plan":
        return emit_json(build_plan(parse_count(argv), ft_capture=ft_capture_enabled(argv)))
    if command == "parse":
        if len(argv) < 3:
            raise SystemExit("usage: mvw_ustack_reserved_imem_chain.py parse RESULT.json [count]")
