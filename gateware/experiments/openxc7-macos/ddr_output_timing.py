...
def verify_direct_sd_outputs(routed: Path, sdf: Path):
    ...
    assert set(outputs) == {0, 1, 2, 3, 7}, outputs
    drivers = {}
    for name, cell in cells.items():
    ...
    for pin, (target, bit) in sorted(outputs.items()):
        ...
        assert cell["type"].startswith("SLICE_FF") and port == "Q", (
            pin,
            source,
            cell["type"],
        )
        assert cell["connections"]["CK"] == [clock], source
        assert int(cell["parameters"].get("IS_CLK_INVERTED", "0"), 2) == 1, source
        delay = delays[source + "/Q", target + "/IN"]
        assert delay < 4000, (pin, delay)
        result.append(
            dict(
                pmod_pin_index=pin,
