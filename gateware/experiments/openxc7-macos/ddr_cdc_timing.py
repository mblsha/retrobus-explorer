...
def verify_native_cdc(routed: Path, sdf: Path, sys_clk_freq=80_000_000):
    ...
    assert "(TIMESCALE 1ps)" in sdf_text
    ...
    for name, bit in sorted(pointers.items()):
        ...
        assert len(source) == len(target) == 1 and len(terminals[bit]) == 2, name
        sn, sp, sc = source[0]
        tn, tp, tc = target[0]
        assert sc["type"].startswith("SLICE_FF") and tc["type"].startswith(
            "SLICE_FF"
        ), name
        source_clock = sc["connections"]["CK"][0]
        target_clock = tc["connections"]["CK"][0]
        assert source_clock != target_clock and target_clock in clock_periods, name
        ...
        assert delay < 0.8 * period, (name, delay, period)
