...
from microsd_host import validate_target
...
def main():
    ...
    a = p.parse_args()
    if not (400000 <= a.clock_hz <= 25000000):
        raise RuntimeError("Check failed: 400000 <= a.clock_hz <= 25000000")
    ...
    if requested != a.clock_hz:
        raise RuntimeError("Check failed: requested == a.clock_hz")
    if "actual clock" not in fields:
        raise RuntimeError("Host did not expose the actual divider clock")
