...
def main() -> int:
    ...
    if not results_xml.is_file():
        print(f"error: Cocotb did not create {results_xml}", file=sys.stderr)
        return 1
    try:
        failures = cocotb_result_failures(results_xml)
    except ET.ParseError as exc:
        print(f"error: invalid Cocotb results XML at {results_xml}: {exc}", file=sys.stderr)
        return 1
    if failures:
        print("error: Cocotb test failures:", file=sys.stderr)
