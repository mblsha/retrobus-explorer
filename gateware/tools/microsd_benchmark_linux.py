...
Wrap with microsd_qualify_linux.py to reject errors and hidden recovery.
"""
...
def main():
    ...
    a = p.parse_args()
    assert 1 <= a.span_mib <= 256 and 1 <= a.repeats <= 10
    ...
    try:
        with mmap.mmap(-1, request_bytes) as buf:
            for repeat in range(a.repeats):
                ...
                for offset in range(0, size, request_bytes):
                    ...
                    count = os.preadv(fd, [buf], offset)
                    io_seconds += time.monotonic() - io_start
                    assert count == request_bytes, (offset, count)
                    digest.update(buf)
                elapsed = time.monotonic() - start
                assert digest.hexdigest() == expected, "read data digest mismatch"
                results.append(
                    dict(
                        repeat=repeat,
                        bytes=size,
        ...
        os.close(fd)
    print(
        json.dumps(dict(passed=True, configuration=vars(a), results=results), indent=2)
    )
