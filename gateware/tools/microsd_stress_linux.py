...
aliases and collateral corruption. Requires microsd_verify_linux_rw.py nearby.
"""
...
def payload(block, generation, seed):
    assert 0 <= block < 65536 and 0 <= generation < 1 << 32
    assert 0 <= seed < 1 << 64
    key = (
...
def main():
    ...
    assert 0 < span <= capacity and args.random_writes >= 0 and args.passes > 0
    ...
    def read(offset, size):
        ...
        try:
            assert os.preadv(fd, [view], offset) == size, ("short read", offset)
            return bytes(view)
        finally:
            view.release()

    def write(offset, data):
        assert offset % BLOCK == 0 and offset + len(data) <= span
        buf[: len(data)] = data
        view = memoryview(buf)[: len(data)]
        try:
            assert os.pwritev(fd, [view], offset) == len(data), ("short write", offset)
        finally:
            view.release()
    ...
    def verify_all(label):
        ...
        for pos in range(0, span, 65536):
            ...
            assert actual == expected, ("full readback mismatch", pos, label)
    ...
    try:
        ...
        for iteration in range(args.passes):
            ...
            for _ in range(args.random_writes):
                ...
                assert read(index * BLOCK, BLOCK) == data, ("random mismatch", index)
                for n, old in neighbors.items():
                    assert read(n * BLOCK, BLOCK) == old, ("neighbor corruption", n)
            os.fsync(fd)
            results.append(
                dict(
            ...
            if untouched is not None:
                assert outside_hash() == untouched, "untouched DDR region changed"
        print(
            json.dumps(
                dict(
