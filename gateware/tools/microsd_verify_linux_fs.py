...
def main():
    ...
    parser.add_argument(
        "--read-only",
        action="store_true",
        help="Verify the previous filesystem test without formatting or writing",
    )
    args = parser.parse_args()
    with qualification(args, filesystem_test=True):
        ...
        try:
            if not args.read_only:
                ...
                (mountpoint / "TESTDIR/ORIGINAL.BIN").rename(
                    mountpoint / "TESTDIR/RENAMED.BIN"
                )
            ...
            for name, data in payloads.items():
                ...
                if actual != data:
                    raise RuntimeError((name, "filesystem readback mismatch"))
                checks.append(
                    dict(
                        path=name,
                        bytes=len(actual),
            ...
            if (mountpoint / "DELETE.TXT").exists():
                raise RuntimeError(
                    "Check failed: not (mountpoint / 'DELETE.TXT').exists()"
                )
            if (mountpoint / "TESTDIR/ORIGINAL.BIN").exists():
                raise RuntimeError(
                    "Check failed: not (mountpoint / 'TESTDIR/ORIGINAL.BIN').exists()"
                )
            run("umount", str(mountpoint))
            mounted = False
            run("fsck.fat", "-n", "/dev/mmcblk1")
