...
def parse_au_pin_map(au_pin_map: Path) -> dict[str, str]:
    if au_pin_map.suffix == ".toml":
        config = tomllib.loads(au_pin_map.read_text())
        pins = config.get("pins")
        if not isinstance(pins, dict) or not pins:
            raise SystemExit(f"error: missing non-empty [pins] table in {au_pin_map}")
        if not all(
            isinstance(symbol, str)
            and re.fullmatch(r"[A-Z0-9_]+", symbol)
            and isinstance(package_pin, str)
            and re.fullmatch(r"[A-Z][0-9]+", package_pin)
            for symbol, package_pin in pins.items()
        ):
            raise SystemExit(f"error: invalid symbolic/package pin entry in {au_pin_map}")
        return dict(pins)

    mapping: dict[str, str] = {}
    for line in au_pin_map.read_text().splitlines():
        m = PIN_DEF_RE.match(line)
        if m:
            mapping[m.group(1)] = m.group(2)
    if not mapping:
        raise SystemExit(f"error: no pin mappings found in {au_pin_map}")
    return mapping
...
def is_shared_constraints_acf(acf: Path) -> bool:
    return "interfaces" in acf.parts and "constraints" in acf.parts
...
def validate_metadata_usage(acf_files: list[Path], acf_metadata: dict[Path, list[tuple[str, str]]]) -> None:
    ...
    for acf in acf_files:
        ...
        if not metadata:
            errors.append(f"{acf}: missing @meta.* header for shared interface constraints file")
            continue
    ...
    if errors:
        ...
        raise SystemExit(f"error: interface constraints metadata validation failed:\n{details}")
...
def render_xdc(
    ...
    *,
    emit_clock_route_overrides: bool = True,
    allow_unconstrained_ports: bool = True,
) -> str:
    lines: list[str] = []
    ...
    if emit_clock_route_overrides and any(signal == "clk" for signal, _, _ in pins):
        lines.append("")
    ...
    if emit_clock_route_overrides and any(signal == "nc" for signal, _, _ in pins):
        lines.append("set_property CLOCK_DEDICATED_ROUTE FALSE [get_nets {nc_IBUF}]")

    if allow_unconstrained_ports:
        lines.append("")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Path to an Au symbolic pin map (TOML or AuPin.kt)",
    )
