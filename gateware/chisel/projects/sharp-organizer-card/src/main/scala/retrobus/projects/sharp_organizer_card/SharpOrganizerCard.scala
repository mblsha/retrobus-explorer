...
class SharpOrganizerCard extends Module {
  ...
  // Instantiate clock wizard to generate 200MHz and 400MHz clocks
  val clkWiz = Module(new ClockWizard)
  clkWiz.io.clk_in100 := io.clk
  ...
  // These need to work across clock domains
  ...
  // High-speed UART transmitters in 400MHz domain
  val txAddr = withClock(clkWiz.io.clk_out400) { 
    Module(new MyUartTx(clkFreq = 400_000_000, baud = 100_000_000, dataWidth = 20))
  }
  val txData = withClock(clkWiz.io.clk_out400) {
    Module(new MyUartTx(clkFreq = 400_000_000, baud = 100_000_000, dataWidth = 8))
  }
  val txMisc = withClock(clkWiz.io.clk_out400) {
    Module(new MyUartTx(clkFreq = 400_000_000, baud = 100_000_000, dataWidth = 11))
  }
  
  // Main logic in 100MHz clock domain
  withClockAndReset(io.clk, rst.asAsyncReset) {
  ...
  // Connect FIFOs to high-speed UART transmitters (in 400MHz domain)
  withClock(clkWiz.io.clk_out400) {
    // Connect to FIFOs (read side at 400MHz)
