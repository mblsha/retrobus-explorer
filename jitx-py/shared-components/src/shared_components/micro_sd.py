...
SPARKFUN_MICRO_SD_SNIFFER_SOURCE = (
    "https://github.com/sparkfun/MicroSD_Sniffer/blob/master/"
    "Hardware/SparkFun_MicroSD_Sniffer.brd"
)
...
MICRO_SD_EDGE_PAD_CENTERS = {
    name: MICRO_SD_EDGE_PAD_CENTERS_BY_NUMBER[number]
    for name, number in MICRO_SD_PIN_NUMBER_BY_PORT.items()
}
...
class Molex1040310811MicroSdSocket(Component):
    ...
    def __init__(self):
        ...
        for port, number in ((self.DETECT_A, 9), (self.DETECT_B, 10)):
            mapping[port] = getattr(self.landpattern, f"p{number}")
        mapping.update(
            {
