...
from src.components import GndTestpads, HDGC60PinFfc, SharpOrganizerHost
...
BOARD_SHAPE = rectangle(62.0, 30.0, radius=1.0)
SIGNAL_AREA = rectangle(61.0, 29.0, radius=0.5)
BUS_PAD_HEIGHT = 5.0


class SharpOrganizerHostSubstrate(Substrate):
...
class SharpOrganizerHostCircuit(Circuit):
    # Direct port of `jitx/sharp-organizer-host.stanza`.
    gnd = Port()
    vcc = Port()
    ...
    def __init__(self):
        ...
        self.bus = SharpOrganizerHost()
        self.gnd_testpads = GndTestpads(diameter=3.0, width=62.0, height=30.0)
        ...
        gnd_net = Net(name="GND") + self.gnd + self.ffc.GND + self.bus.GND + self.gnd_testpads.GND
        ...
        self += Silkscreen(Text(f"(c) mblsha {BOARD_DATE}", 1.5).at(0.0, -1.5), side=FeatureSide.Top)


class SharpOrganizerHostBoard(Board):
    shape = BOARD_SHAPE
    signal_area = SIGNAL_AREA


class SharpOrganizerHostDesign(Design):
    substrate = SharpOrganizerHostSubstrate()
    board = SharpOrganizerHostBoard()
    circuit = SharpOrganizerHostCircuit()
