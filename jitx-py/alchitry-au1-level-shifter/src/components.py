...
class SaleaeProbeHeaderLandpattern(Landpattern):
    # Exact port of the archived `PIN_HDR_8` footprint reused by Saleae boards.
    def __init__(self):
        ...
        self.outline = Silkscreen(
            Polyline(
                0.153,
                [(-2.54, -5.08), (-2.54, 5.08), (2.54, 5.08), (2.54, -5.08), (-2.54, -5.08)],
            )
        )


class SaleaeProbeHeader2x4(Component):
    GND = Port()
    p0 = Port()
    p1 = Port()
    p2 = Port()
    ...
    def __init__(self):
        self.landpattern = SaleaeProbeHeaderLandpattern()
